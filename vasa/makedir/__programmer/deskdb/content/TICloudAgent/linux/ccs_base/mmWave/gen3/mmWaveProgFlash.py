...
xWRL64xx_PART_NUM = "WRL64" # common root for xWRL64 device names
xWRL14xx_PART_NUM = "WRL14" # common root for xWRL14 device names
IWRL64xx_PART_NUM = "IWRL64"
AWRL64xx_PART_NUM = "AWRL64"
IWRL14xx_PART_NUM = "IWRL14"
AWRL14xx_PART_NUM = "AWRL14"

#Add PartNums here for support
PartNumSupported = [IWRL64xx_PART_NUM,AWRL64xx_PART_NUM,IWRL14xx_PART_NUM,AWRL14xx_PART_NUM,xWRL64xx_PART_NUM,xWRL14xx_PART_NUM]
...
#                      "fileType" : ["META_IMAGE1", "META_IMAGE2", "META_IMAGE3", "META_IMAGE4"]
#                    },
# xWRL64xx_PART_NUM : { "headers" : [0x5254534D],
#                      "fileType" : ["META_IMAGE1", "META_IMAGE2", "META_IMAGE3", "META_IMAGE4"]
#                    },                
#                    }, # end of PRE_PG3_KEY
# AWR_POST_PG3_KEY : {
...
class BootLdr:
    ...
    def _comm_open(self):
        ...
        #----------TEST-------------
        if self.comm.isOpen():
            #Serial.flushInput() is deprecated to Serial.reset_input_buffer()
    ...
    def _send_command(self,data):
        ...
        if(not ackStatus):
            ...
            try:
                ...
                self._trace_msg(TRACE_LEVEL_ERROR,str(e))
            self._trace_msg(TRACE_LEVEL_ERROR,"Nack Received, Err Code: {}".format(st))
        self._trace_msg(TRACE_LEVEL_DEBUG,"<--- Send command")
        return ackStatus
    ...
    def _get_hex_string(self,value):
        self._trace_msg(TRACE_LEVEL_INFO,"##### IN Hex")
        hexString = hex(value)
        if(hexString[-1] == 'L'):
        ...
        self._trace_msg(TRACE_LEVEL_INFO,"##### OUT Hex")
        return "0x" + hexString
    
    def _send_SRAM_file_close_command(self, file_id):
        ...
        try:
            ...
            self._trace_msg(TRACE_LEVEL_ERROR,str(e))
        self._trace_msg(TRACE_LEVEL_INFO,"Bootloader Status: {}".format(st))
        if(st != "0x0000000000000000"):
            self._trace_msg(TRACE_LEVEL_ERROR, "MetaImage Download Err")
        else:
    ...
    def _getFileHeaderList(self):
        return FileHeaders[self.partNum[1:6]]["headers"]

    def _getFileTypeList(self):
        return FileHeaders[self.partNum[1:6]]["fileType"]
    ...
    def download_file(self,filename,file_id,mirror_enabled,max_size,storage, imageProgList):
        ...
        result = True
        #Payload size is genralized to 8, previously 4 for SRAM and 1 for SFLASH
        self.cmdStatusSize = 8
                ...
                if (self._send_start_download(file_id,fSize,max_size,mirror_enabled,storage)):
                    ...
                    while (offset < fSize):
                        ...
                        if (c is True):
                            ...
                            result = False
                            break
                if(storage == "SRAM" and result):
                    sendStatus = self._send_SRAM_file_close_command(file_id)
                    if(sendStatus == False):
                        result = False
                elif(result):
    ...
    def isPartNumSupported(self, partNum):
        return partNum[0:6] in PartNumSupported
    ...
    #     return True
    # def determineSRVersion(self):
    #     self._trace_msg(TRACE_LEVEL_DEBUG, "->Entering determineSRVersion method")
    #     # ----- TEST ----
    #     serialStub.PARTNUM=self.partNum
    #     # ----- TEST ----
    #     # get the version from the device.
    #     versionRead = self.GetVersion()
    #     if (versionRead == ""):
    #         self._trace_msg(TRACE_LEVEL_DEBUG,"<-Exit determineSRVersion method, failure")
    #         return False
    #     if (versionRead in StatusResponseVerNew):
    #         # new status response version
    #         self.cmdStatusSize = 8

    #     else:
    #         # old Staus response version
    #         self.cmdStatusSize = 1
    #     self._trace_msg(TRACE_LEVEL_DEBUG,"<-Exit determineSRVersion method")
    #     return True
