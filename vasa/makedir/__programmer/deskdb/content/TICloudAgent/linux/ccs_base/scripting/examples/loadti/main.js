...
function run()
{
	...
	if (testEnv.cpu != null){
	    try{
		    printTrace("Specifying CPU: " + testEnv.cpu );
		    debugSession = debugServer.openSession("*", testEnv.cpu);
	    }
	    catch(error) {
		    printTrace("Invalid CPU Name: " + testEnv.cpu );
		    printTrace("Switching to the default CPU");
