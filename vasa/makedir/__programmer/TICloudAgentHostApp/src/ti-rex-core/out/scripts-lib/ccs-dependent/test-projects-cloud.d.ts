...
export const command: "test-projects-cloud [options]";
export const describe: "Test all the projects in tirex in a cloud environment, verifying they import and build successfully. Deletes all projects from the workspace then runs an import & build on each project, deleting all projects from the workspace in between each import & build.";
...
export type testProjectsCloudCallback = (error: Error, result: {
    total: number;
    failed: number;
    log: string;
}) => any;
