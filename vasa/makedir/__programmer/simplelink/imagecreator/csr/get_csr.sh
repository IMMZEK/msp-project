...
# v.0.4
#
# Usage : ./get_csr.sh <device(CC3220S, CC3220SF, CC3235S, CC3235SF)> <port(COM+number)>
echo
echo get_csr.sh v.0.4
echo
# directory of script
SCRIPTFILEPATH=$(cd "$(dirname "$0")"; pwd)
IMAGECREATORPATH=$SCRIPTFILEPATH/../bin
SDKINSTALLPATH=/home/user/ti/simplelink_cc32xx_sdk_2_20_00_04_eng
SPNAME=sp_3.8.0.0_2.0.0.0_2.2.0.6.bin
SP_PATH=$SDKINSTALLPATH/tools/cc32xx_tools/servicepack-cc3x20
CERT_LST=certcatalogPlayGround20160911.lst
CERT_LST_BIN=certcatalogPlayGround20160911.lst.signed_3220.bin
CERT_LST_PATH=$SDKINSTALLPATH/tools/cc32xx_tools/certificate-playground
...
if [[ ! $1 ]]; then
    echo
    echo Error: Device name not specified. Supports only CC3220S/CC3220SF/CC3235S/CC3235SF
    echo
exit 1
fi
if [ "$1" != "CC3220S" ] && [ "$1" != "CC3220SF" ] && [ "$1" != "CC3235S" ] && [ "$1" != "CC3235SF" ] ; then
    echo
    echo Error: Bad device name. Should be CC3220S/CC3220SF/CC3235S/CC3235SF
    echo
exit 1
fi
...
echo Creating A New ImageCreator Project
$RUNCMD -q project new --name $PROJNAME --device $PROJDEVICE --description "project for csr" --overwrite


echo Setting Service Pack
$RUNCMD -q project set_sp  --name $PROJNAME --file $SP_PATH/$SPNAME

echo Setting Certificate Store
$RUNCMD project set_certstore --name $PROJNAME --file "$CERT_LST_PATH/$CERT_LST" --sign "$CERT_LST_PATH/$CERT_LST_BIN"

echo Adding Development PlayGround Root CA Cert [ RootCA ]
$RUNCMD -q project add_file  --name $PROJNAME --fs_path "dummy-root-ca-cert" --file "$DUMMY_CERT_PATH/$DUMMY_CERT_NAME"

echo Adding MCU Image [ Signed ]
$RUNCMD -q project add_file  --name $PROJNAME --mcu --file "$SCRIPTFILEPATH/$PROJDEVICE/$BINFILENAME" --priv "$DUMMY_CERT_PATH/$DUMMY_KEY_NAME" --cert "$DUMMY_CERT_NAME"

echo Program The Image
if [[ ! $COMPORT ]]; then
    $RUNCMD -q project program --name $PROJNAME
    else
    $RUNCMD -q project program --port $COMPORT --name $PROJNAME
fi
