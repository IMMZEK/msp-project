...
#include <istream>
    ...
    defined (__DMC__) || defined (__TI_COMPILER_VERSION__)
#  include <stdint.h>
#endif
...
static double _Stl_atod(char *buffer, ptrdiff_t ndigit, int dexp) {
  ...
  _STLP_STATIC_ASSERT(sizeof(uint64) >= sizeof(long double))
  return drep.val;
}
...
#ifndef __linux__
static long double _Stl_string_to_double(const char *s) {
  typedef numeric_limits<long double> limits;
...
#elif !defined (__linux__) && !defined (__MINGW32__) && !defined (__CYGWIN__) && \
      !defined (__BORLANDC__) && !defined (__DMC__) && !defined (__HP_aCC)
