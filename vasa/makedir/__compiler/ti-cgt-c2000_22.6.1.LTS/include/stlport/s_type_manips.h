...
struct _Copyable {
  ...
  enum __attribute__((__packed__)) { _Ret = __type2bool<_RetT>::_Ret };
};
#  endif
#else
template <class _Src, class _Dst>
...
#  if !defined (__BORLANDC__) || (__BORLANDC__ >= 0x590)
  ...
  enum __attribute__((__packed__)) { value = (sizeof(char) == sizeof(_H::_Test(false, _H::_MakeSource()))) };
#  else
  enum __attribute__((__packed__)) { _Is1 = __type2bool<_IsConst<_Src>::_Ret>::_Ret };
  enum __attribute__((__packed__)) { _Is2 = _IsConvertible<_UnConstPtr<_Src>::_Type, _UnConstPtr<_Dst>::_Type>::value };
  enum __attribute__((__packed__)) { value = _Is1 ? 0 : _Is2 };
#  endif
  typedef typename __bool2type<value>::_Ret _Ret;
};
